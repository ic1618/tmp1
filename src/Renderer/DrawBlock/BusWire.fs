...
type Segment = 
    {
        ...
        End: XYPos
        Dir: Orientation
        HostId: ConnectionId // which wire the segment belongs to
        /// List of x-coordinate values of segment jumps. Only used on horizontal segments.
        JumpCoordinateList: list<float * SegmentId>
        Draggable : bool // normally true, but false for the two segments at either end of wire. these two segments alw have length 0. 
    }

/// currently always a set of 7 segments, even if they are 3seg or 5seg wires
/// currently alternating horizontal and vertical segment. 
/// if you want a fully horizontal wire, just set the vert segment in btwn hori segments to be 0
type Wire =
    {
        Id: ConnectionId 
...
// store jumpcoordlist in seg or wire?
// only neeeded in horizontal segment, but u need to know if seg has jumps on it
...
//         HostId: ConnectionId
//         JumpCoordinateList: list<float * SegmentId> 
//         Draggable : bool // perhaps change this away from how it is currently working. more complex type structure vs more complex code.
//     }
